...
<body>
    <div style="width : 1200px; margin: 0 auto">
        <div class="container-fluid">
            ...
            <div class="row">
                <jsp:include page="../commons/userMyPageSideNav.jsp"></jsp:include>
                <div class="col-8">여기는 유저 요청내역입니다.
            		<!-- 타이틀 -->
                	<div class="row">
                		<div class="col">
             				<h3> 요청 내역 </h3>
							  	...
							  	<c:forEach items="${dataList }" var="data" varStatus="status"> 
								    <tr class = "text-center">
								      ...
								      <td>
								      	<c:choose>
			                				<c:when test="${data.memberVo.member_profileimage_link == null}">
				                				<img id = "userRequestPageProfileImg" class="img-fluid center-block" src="../resources/img/noneProfileImg.jpg">
			                				</c:when>
			                				<c:otherwise>
			                					<img id = "userRequestPageProfileImg" class="img-fluid center-block" src="/uploadFiles/${data.memberVo.member_profileimage_link}">
			                				</c:otherwise>
		               					</c:choose>
								      </td>
								      ...
								      <c:choose>
									      ...
									      <td><a href="./paymentCompletePage?order_no=${data.orderVo.order_no}">결제 상세보기</a></td>
								      	</c:when>
								      	<c:when test="${data.orderVo.order_state eq 'reject'}">
									      <td><span>반려 됨</span></td>
									      <td><a href="./paymentCompletePage?order_no=${data.orderVo.order_no}">결제 상세보기</a></td>
								      	</c:when>
									      ...
									      <td><a href="../member/stylingResponseCard?order_no=${data.orderVo.order_no}">답변 상세보기</a></td>
								      	</c:when>
								      	<c:when test="${data.orderVo.order_state eq 'complete'}">
									      <td><span>리뷰 완료</span></td>
									      <td><a href="../member/readStylistCard?order_no=${data.stylist_CardVo.stylist_card_no}">리뷰 보러가기</a></td>
								      	</c:when>
								      	<c:otherwise>
								      	  <td><span> 에러 </span></td>
								      	</c:otherwise>
